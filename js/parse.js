...
function createCogCSVs(cleanJson){
    const retCSVs = [];
    var cogs = cleanJson.account.cogs;


    var debugExport=JSON.stringify(cogs,null,"\t");

    var emptiesExport="empties_x,empties_y\n";

    var cogsDataExport="cog type,name,build_rate,flaggy_rate,exp_mult,exp_rate,build_rate_boost,flaggy_rate_boost,flaggy_speed,exp_rate_boost\n";
    ...
    for(i=0;i<cogs.length;i++){
        ...
        if(i<96){
            ...
            if(cogs[i]["name"]=='Blank')
                emptiesExport+=x+","+y+"\n";
            x++;
                        
            if(cogs[i]["name"]!="Blank" || cogs[i]["data"]!="none")
                cogsDataExport+=AddCog(cogs[i]);
        }
        //COG PRODUCTION
        else if(i<108){}
        //COG STORAGE
    ...
    retCSVs[0]=cogsDataExport;
    retCSVs[1]=emptiesExport;
    retCSVs[3]=debugExport; //DEBUG

    return retCSVs;
}
//Create Cog in csv-format
function AddCog(cog)
{
    var cogsDataExport="";

    //cog type / character name
    if(cog["name"].startsWith("Player_")){
        cogsDataExport+="Character,"+cog["name"].split("_")[1]+",";
    }
    else{
        if(cog.data.hasOwnProperty("h"))
            cogsDataExport+=translateCogNames(cog.data.h)+",,";
        else
            cogsDataExport+="Cog,,";

    }
    //build_rate
    if(cog.data.hasOwnProperty("a"))
        cogsDataExport+=cog.data.a;
    cogsDataExport+=",";

    //flaggy_rate
    if(cog.data.hasOwnProperty("c"))
        cogsDataExport+=cog.data.c;
    cogsDataExport+=",";

    //exp_mult
    if(cog.data.hasOwnProperty("d"))
        cogsDataExport+=cog.data.d/100.0;
    cogsDataExport+=",";
    ...
    //build_rate_boost
    if(cog.data.hasOwnProperty("e"))
        cogsDataExport+=cog.data.e/100.0;
    cogsDataExport+=",";            

    //flaggy_rate_boost
    if(cog.data.hasOwnProperty("g"))
    ...
    return cogsDataExport;
}
function translateCogNames(cogname){
